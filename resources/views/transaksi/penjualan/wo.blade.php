...
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-default">
			<form name="form_wo" onsubmit="return validateForm()" action="wo/store" method="post">
				{{ csrf_field() }}
					...
					<div class="panel panel-success">
						<div class="panel-body">
							<div class="row">
								<div class="col-md-6">
									<div style="padding: 5px" class="form-group{{ $errors->has('nama') ? 'has-error' : '' }}">
										<label class="col-md-3">Tanggal Estimasi</label>
										<div class="col-md-3">    
											<input type="text" id="tanggal_estimasi" value="{{$date_estimasi}}" name="tanggal_estimasi" class="form-control datepicker">   
								...
								<div class="col-md-6">
									...
									<div class="form-group">
										<div style="padding: 5px" class="form-group{{ $errors->has('nama') ? 'has-error' : '' }}">
											...
											<div class="col-md-6" style="text-align: right;">    
												Rp. <label class="control-label" id="sisa"></label>    
											</div>
										</div>
									</div>
								</div>
...
<script>
	...
	function AddRow()
	{
        ...
        $('#dp').removeAttr('disabled'); //Enable Button Submit
	}
	...
	// 		splitid = id.split('_');
	// 		index = splitid[1];
	// 		var supplierID = $('#kode_'+index).val();
	// 		if (supplierID){
	// 		console.log(supplierID);
 //        		$.ajax({
 //                    url: '/wo/kode_barcode/'+supplierID,
 //                    type: "GET",

	// 			beforeSend: function(){
	// 				$("#kode_"+index).css("background","#FFF no-repeat 165px");
	// 			},
 ...
 //        	}
	...
	$(document).ready(function(){
	   ...
	   //  			$("#myModal").modal("show");	
		...
		$('#dp').keyup(function() { 			
			...
			var dp = $(this).val().split(',').join("");
	        var dpAngka = toAngka('Rp'+dp);
			
			var result = sub_total - dpAngka;

			if(dpAngka > sub_total){
				checkDP = false;
				$('#showNotifDp').show();
				$('#showNotifDp').text("DP melebihi total harga barang");
			}else{
				checkDP = true;
				$('#showNotifDp').hide();
			    $('#sisa').text(toRp(result));
			    // $('#sisa').val(result);	
				console.log(result);
			}
		});
	...
	function update_amounts()
	{
	    ...
	    var discount = document.forms["form_wo"]["discount"].value;
	    var pihak_ketiga = document.forms["form_wo"]["pihak_ketiga"].value;
	    var discountAngka = toAngka('Rp'+discount);
	    var Angka = toAngka('Rp'+pihak_ketiga);
	    if(isNaN(discountAngka)){
	    	discountAngka = 0;
	...
	function validateForm() {
		var bool = false;
	    var tanggal = document.forms["form_wo"]["tanggal"].value;
	    var sales = document.forms["form_wo"]["employee_id"].value;
	    var tanggal_estimasi = document.forms["form_wo"]["tanggal_estimasi"].value;
	    var no_nota = document.forms["form_wo"]["no_nota"].value;
	    var dp = document.forms["form_wo"]["dp"].value;
	    if (tanggal == "") {
	    	...
	    	if(sales == "0"){
		    	...
		    	if(tanggal_estimasi == ""){
			    	...
			    	if (no_nota == "") {
				    	...
				    	if (dp == "") {
				    		$('#showNotifDp').show();
				    		$('#showNotifDp').text("DP Tidak Boleh Kosong");
				    		bool = false;
				    	}else{
				    		$('#showNotifDp').hide();
				    		if (!checkDP){
