			...
			<div class="panel panel-default">
				...
				<div class="col-md-12">
					<table width="100%">	
						<tr>
							...
							<td width="25%">
                                <div class="form-group col-md-12">
									<input type="radio" name="chpas" id="option1"/>&nbsp Kode PO &nbsp &nbsp
									<input type="radio" name="chpas" id="option2" checked/>&nbsp Supplier
                                </div>
							</td>
							<td width="60%"></td>
						</tr>
						...
						{{ csrf_field() }}
						<tr id="child_option_1" style="display:none">
				...
				<br>
				<div class="col-md-12">
				<form onsubmit="return validateForm()" name="form_penerimaan" method="post" action="updatesupplier">
				{{ csrf_field() }}
				<input type="hidden" name="supplier_id" value="{{$supplier_mine->id}}">
					...
					<tr id="child_option_3" style="display: none;">
						<th style="text-align: right;" height="40px"><label>Tanggal PO</label></th>
						<th style="text-align: center;"><label> : </label></th>
						<td style="text-align: left;"><label name="tanggalPo" id="tanggalPo">{{$tanggal_preorder->tanggal_estimasi}}</label></td>
						<td></td>
					</tr>
					<tr>
						<th style="text-align: right;" height="40px"><label>Supplier</label></th>
						<th style="text-align: center;"><label> : </label></th>
						<td style="text-align: left;"><label name="pemasok" id="pemasok"></label>{{$supplier_mine->nama}}</td>
						<td></td>
					</tr>
				</table>
				</div>
				...
				<div class="col-md-12" align="center">
					...
					<tr class="realBody">
						...
						<input type="hidden" value="{{ $datas->pre_order_id }}" name="pre_order_id[]" style="text-align: center;" readonly>
						<input type="hidden" value="{{ $datas->pre_order_out_id }}" name="pre_order_out_id[]" value="{{ $datas->nama_barang}}" style="text-align: center;" readonly>
						<td style="text-align: center;"><input class="form-control" type="text" value="{{ $datas->barang_id }}" name="barang_id[]" value="{{ $datas->nama_barang }}" style="text-align: center;color: black;" readonly></td>
						<td style="text-align: center;"><input class="form-control" type="text" name="nama_barang[]" value="{{ $datas->nama_barang }}" style="text-align: left;color: black;" readonly></td>
						<input class="form-control" type="hidden" name="harga_po[]" value="{{ $datas->harga_po }}" style="text-align: left;color: black;" readonly>
						<td style="text-align: center;"><input class="form-control qty" type="text" name="qty[]" value="{{ $datas->jumlah_pesan }}" style="text-align: right;color: black;" readonly></td>
				...
				<table width="90%" align="center">
					<tr>
						...
						<th width="17%" style="text-align: left;"><label name="jumlahBarang" id="jumlahBarang">{{$total_qty}}</label></th>
						<th width="5%"></th>
					</tr>
					<tr>
				...
				<br>
					<table width="90%" align="center">
						<tr>
							...
							<th width="17%" style="text-align: left;"><button class="btn btn-info btn-block">Save</button>
							<span style="color: red; text-align:left; display:none" id="chcekValidasi"></span></th>
							<th width="5%"></th>
						</tr>
					</table>
				</form>	
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on('input', '.qty_diterima', function(){
			...
			if (qty_diterima < 0) {
				$('#checkQtyDiterima_'+index).text("Qty Tidak Boleh Lebih kecil dari nol");
